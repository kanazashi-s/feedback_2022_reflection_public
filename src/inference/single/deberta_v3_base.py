...
class DeBERTaV3BaseModel(pl.LightningModule):
    def __init__(self):
        super().__init__()
        config_path = DeBERTaV3BaseCFG.output_dir / "config.pth"
    ...
    def configure_optimizers(self):
        ...
        optimizer = AdamW(
            self.model.parameters(), eps=GeneralCFG.eps, betas=DeBERTaV3BaseCFG.betas
        )
        num_train_steps = int(GeneralCFG.n_fold / DeBERTaV3BaseCFG.batch_size * self.trainer.max_epochs)
...
def inference():
    ...
    for fold in GeneralCFG.train_fold:
        input_dir = DeBERTaV3BaseCFG.output_dir
    ...
    make_submission_csv(predictions, DeBERTaV3BaseCFG.output_dir)
