...
def make():
    output_path = Path("/workspace", "data", "processed", "reg_token")
    ...
    for seed in GeneralCFG.seeds:
        ...
        # insert regression tokens
        train_fold["full_text"] = train_fold["full_text"].apply(insert_reg_tokens)
...
def insert_reg_tokens(full_text: str) -> str:
    """
    与えられた文章の冒頭に、
    cohesion syntax vocabulary phraseology grammar conventions [SEP]
    を挿入する。
    """
    return " cohesion syntax vocabulary phraseology grammar conventions [SEP] " + full_text
