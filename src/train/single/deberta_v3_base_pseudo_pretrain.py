...
def train(run_name: str):
    ...
    for seed in GeneralCFG.seeds:
        ...
        for fold in GeneralCFG.train_fold:
            ...
            model.learning_rate = DeBERTaV3BaseCFG.lr / 4
            DeBERTaV3BaseCFG.num_cycles = 0
            trainer.fit(model, pseudo_data_module)
            torch.cuda.empty_cache()
            gc.collect()

            # fine-tune using train data
            ...
            model.learning_rate = DeBERTaV3BaseCFG.lr
            DeBERTaV3BaseCFG.num_cycles = 0.5
...
if __name__ == "__main__":
    DeBERTaV3BaseCFG.output_dir = Path("/workspace", "output", "single", "deberta_v3_base", "pseudo_1")
    oof_score_seed_mean, columns_score_seed_mean = train(f"pseudo_1")
